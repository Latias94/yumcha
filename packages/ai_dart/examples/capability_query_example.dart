...
class ProviderCapabilityQuery {
  ...
  static String? getBestProviderFor(Set<LLMCapability> requiredCapabilities) {
    ...
    for (final factory in factories.values) {
      if (requiredCapabilities.every((cap) => factory.supportedCapabilities.contains(cap))) {
  ...
  static void printCapabilityMatrix() {
    ...
    final capHeaders = allCapabilities.map((cap) => cap.name.substring(0, 4).padRight(5)).join('');
...
void main() async {
  ...
  print('OpenAI supports embedding: ${ProviderCapabilityQuery.providerSupports('openai', LLMCapability.embedding)}');
  print('Anthropic supports embedding: ${ProviderCapabilityQuery.providerSupports('anthropic', LLMCapability.embedding)}');
  print('Ollama supports model listing: ${ProviderCapabilityQuery.providerSupports('ollama', LLMCapability.modelListing)}');
  
  print('\n=== Find Providers by Capability ===\n');
  
  // 3. Find providers with specific capabilities
  final embeddingProviders = ProviderCapabilityQuery.getProvidersWithCapability(LLMCapability.embedding);
  print('Providers with embedding: $embeddingProviders');
  
  final reasoningProviders = ProviderCapabilityQuery.getProvidersWithCapability(LLMCapability.reasoning);
  print('Providers with reasoning: $reasoningProviders');
  
  final ttsProviders = ProviderCapabilityQuery.getProvidersWithCapability(LLMCapability.textToSpeech);
  ...
  final bestForEmbedding = ProviderCapabilityQuery.getBestProviderFor(chatAndEmbedding);
  print('Best provider for chat + embedding: $bestForEmbedding');
  
  final chatAndReasoning = {LLMCapability.chat, LLMCapability.reasoning};
  final bestForReasoning = ProviderCapabilityQuery.getBestProviderFor(chatAndReasoning);
  print('Best provider for chat + reasoning: $bestForReasoning');
  
  final allVoice = {LLMCapability.chat, LLMCapability.textToSpeech, LLMCapability.speechToText};
...
Future<void> demonstrateSmartSelection() async {
  ...
  if (ProviderCapabilityQuery.providerSupports('ollama', LLMCapability.chat)) {
    ...
    if (ProviderCapabilityQuery.providerSupports('ollama', LLMCapability.embedding)) {
...
void extensionExample() {
  ...
  print('OpenAI supports embedding: ${'openai'.supports(LLMCapability.embedding)}');
  print('Anthropic supports reasoning: ${'anthropic'.supports(LLMCapability.reasoning)}');
  ...
  final voiceCapabilities = {LLMCapability.textToSpeech, LLMCapability.speechToText};
  print('OpenAI supports voice: ${'openai'.supportsAll(voiceCapabilities)}');
  print('Anthropic supports voice: ${'anthropic'.supportsAll(voiceCapabilities)}');
}
