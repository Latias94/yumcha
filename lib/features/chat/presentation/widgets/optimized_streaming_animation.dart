...
class OptimizedStreamingAnimation extends StatefulWidget {
  ...
  @override
  State<OptimizedStreamingAnimation> createState() => _OptimizedStreamingAnimationState();
}

class _OptimizedStreamingAnimationState extends State<OptimizedStreamingAnimation>
  ...
  void _checkPerformanceAndAdjust() {
    if (_averageFrameTime > 33.33) { // 低于30fps
      // 可以在这里实施性能优化策略
      // 例如：降低动画复杂度、减少更新频率等
      debugPrint('Performance warning: Average frame time ${_averageFrameTime.toStringAsFixed(2)}ms');
    }
  }
          ...
          text: TextSpan(
            children: [
              ...
              if (widget.showCursor && widget.isStreaming)
                TextSpan(
                  text: '|',
                  style: (widget.cursorStyle ?? widget.style ?? DefaultTextStyle.of(context).style).copyWith(
                    color: (widget.cursorStyle?.color ?? widget.style?.color ?? Colors.black)
