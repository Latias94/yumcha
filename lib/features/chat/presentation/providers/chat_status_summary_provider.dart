...
/// 聊天准备状态Provider（向后兼容）
final chatReadyStateProviderCompat = Provider<bool>((ref) => 
  ref.watch(chatStatusSummaryProvider).isReady);

/// 是否有流式消息Provider（向后兼容）
final hasStreamingMessagesProviderCompat = Provider<bool>((ref) => 
  ref.watch(chatStatusSummaryProvider).hasStreamingMessages);

/// 消息数量Provider（向后兼容）
final messageCountProviderCompat = Provider<int>((ref) => 
  ref.watch(chatStatusSummaryProvider).totalMessages);
...
/// 聊天错误列表Provider（新增）
final chatErrorsProvider = Provider<List<ChatError>>((ref) => 
  ref.watch(chatStatusSummaryProvider).errors);

/// 聊天警告Provider（新增）
final chatWarningsProvider = Provider<List<String>>((ref) => 
  ref.watch(chatStatusSummaryProvider).warnings);

/// 聊天连接状态Provider（新增）
final chatConnectionStatusProvider = Provider<bool>((ref) => 
  ref.watch(chatStatusSummaryProvider).isConnected);

/// 聊天可发送状态Provider（新增）
final chatCanSendMessageProvider = Provider<bool>((ref) => 
  ref.watch(chatStatusSummaryProvider).canSendMessage);

/// 聊天需要注意Provider（新增）
final chatNeedsAttentionProvider = Provider<bool>((ref) => 
  ref.watch(chatStatusSummaryProvider).needsAttention);

/// 聊天健康状态Provider（新增）
final chatHealthStatusProvider = Provider<HealthStatus>((ref) => 
  ref.watch(chatStatusSummaryProvider).healthStatus);

/// 聊天性能指标Provider（新增）
final chatPerformanceProvider = Provider<ChatPerformanceMetrics?>((ref) => 
  ref.watch(chatStatusSummaryProvider).performance);
