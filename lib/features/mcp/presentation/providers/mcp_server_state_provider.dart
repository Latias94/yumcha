...
final mcpServerStateProvider =
    Provider.family<McpServerState, String>((ref, serverId) {
...
final mcpServerStatusProvider = Provider.family<McpConnectionStatus, String>(
    (ref, serverId) => ref.watch(mcpServerStateProvider(serverId)).status);

/// MCP服务器错误Provider（向后兼容）
final mcpServerErrorProvider = Provider.family<String?, String>(
    (ref, serverId) =>
        ref.watch(mcpServerStateProvider(serverId)).primaryError);

/// MCP服务器工具Provider（向后兼容）
final mcpServerToolsProvider = Provider.family<List<McpTool>, String>(
    (ref, serverId) => ref.watch(mcpServerStateProvider(serverId)).tools);
...
final mcpServerConnectionProvider = Provider.family<bool, String>(
    (ref, serverId) => ref.watch(mcpServerStateProvider(serverId)).isConnected);

/// MCP服务器可用性Provider（新增）
final mcpServerUsabilityProvider = Provider.family<bool, String>(
    (ref, serverId) => ref.watch(mcpServerStateProvider(serverId)).isUsable);

/// MCP服务器健康状态Provider（新增）
final mcpServerHealthProvider = Provider.family<HealthStatus, String>(
    (ref, serverId) =>
        ref.watch(mcpServerStateProvider(serverId)).healthStatus);

/// MCP服务器需要注意Provider（新增）
final mcpServerNeedsAttentionProvider = Provider.family<bool, String>(
    (ref, serverId) =>
        ref.watch(mcpServerStateProvider(serverId)).needsAttention);

/// MCP服务器工具数量Provider（新增）
final mcpServerToolCountProvider = Provider.family<int, String>(
    (ref, serverId) => ref.watch(mcpServerStateProvider(serverId)).toolCount);

/// MCP服务器错误列表Provider（新增）
final mcpServerErrorsProvider = Provider.family<List<McpError>, String>(
    (ref, serverId) => ref.watch(mcpServerStateProvider(serverId)).errors);

/// MCP服务器警告Provider（新增）
final mcpServerWarningsProvider = Provider.family<List<String>, String>(
    (ref, serverId) => ref.watch(mcpServerStateProvider(serverId)).warnings);

/// MCP服务器能力Provider（新增）
final mcpServerCapabilitiesProvider =
    Provider.family<McpServerCapabilities?, String>((ref, serverId) =>
        ref.watch(mcpServerStateProvider(serverId)).capabilities);

/// MCP服务器性能指标Provider（新增）
final mcpServerMetricsProvider = Provider.family<McpServerMetrics?, String>(
    (ref, serverId) => ref.watch(mcpServerStateProvider(serverId)).metrics);

/// MCP服务器重连能力Provider（新增）
final mcpServerCanReconnectProvider = Provider.family<bool, String>(
    (ref, serverId) =>
